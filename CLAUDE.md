...
### サイト情報
- **タイトル**: RecoWork - 日々頑張る人たちのSNS
- **説明**: 日々の頑張りを共有して、みんなで継続的に頑張っていくためのSNS
- **ファビコン**: `app/favicon.ico`（カスタムアイコン設定済み）
- **言語**: `ja`（日本語）
...
2. ✅ APIルート作成
   ...
   - モデル: `claude-haiku-4-5-20251001`（高速・高品質レスポンス用）
...
### 名前自動生成トリガー修正 (2026-02-08)
1. ✅ displayNameトリガーの修正
   - 新規ユーザー登録時に`display_name`をNULLで作成するよう変更
   - 以前はメールアドレスの@マーク前を自動設定していたため、AI名前生成が動作しなかった
...
### AIメッセージ生成品質向上 (2026-02-09)
1. ✅ モデルアップグレード
   - `claude-3-haiku-20240307` → `claude-haiku-4-5-20251001` に変更
   - 指示追従能力が大幅に向上、コスト帯は同等
2. ✅ 「1日連続」問題の修正
   - streak が2以上のときだけ統計に含めるよう変更
   - streak=1のときAIはストリーク情報を見ないため「1日連続」を生成しなくなる
3. ✅ カテゴリカウント混同の修正
   - 統計セクションを「今回のカテゴリに関する統計」と「全カテゴリの累計統計（参考情報）」に明確分離
   - 全体カウントに「今回の記録には直接関係なし」と明記し、AIの混同を防止
4. ✅ プロンプト構造改善
   - 単一userメッセージ → system（役割・ルール・禁止事項）+ user（動的データ）に分離
   - 数値正確性・連続日数・表現多様性に関する厳守ルールをsystemプロンプトに明記
   - 具体的な悪い例を含む禁止事項（例：「全カテゴリ20件でも20回目の筋トレとは言わない」）
5. ✅ 履歴データ最適化
   - 取得件数: 50件 → 10件に削減
   - 過去AIメッセージ: 全件埋め込み → 直近5件のみ別枠で管理
